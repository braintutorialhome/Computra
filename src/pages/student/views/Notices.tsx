...
export default function StudentNotices({ student }: { student: Student }) {
  const { notices, testResults, tests } = useStorage();
  const [activeTab, setActiveTab ] = useState<'notices' | 'results'>('notices');
        ...
        <div className="flex p-1.5 glass rounded-2xl border-white/5">
           <button 
             ...
             className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
               activeTab === 'notices' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 hover:text-slate-300'
             }`}
           >
             Notices
           </button>
           <button 
             ...
             className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
               activeTab === 'results' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-900/40' : 'text-slate-500 hover:text-slate-300'
             }`}
           >
             Assessments
           </button>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'notices' ? (
          ...
          <motion.div 
            ...
            className="max-w-5xl space-y-10"
          >
            <div className="glass rounded-[60px] border border-white/5 overflow-hidden shadow-2xl divide-y divide-white/5">
              <div className="p-12 border-b border-white/5 bg-white/[0.01] flex items-center gap-4">
                 <Trophy className="text-purple-400" size={32} />
                 <div>
                    <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Assessment History</h3>
                    <p className="text-xs font-black text-slate-600 uppercase tracking-widest mt-1">Verified scorecards for {student.name}</p>
                 </div>
              </div>
              
              {studentResults.slice().reverse().map(r => {
                const test = tests.find(t => t.id === r.testId);
                const percent = Math.round((r.score / r.totalQuestions) * 100);
                return (
                  <div key={r.id} className="p-12 flex items-center justify-between group hover:bg-white/[0.02] transition-all">
                     <div className="flex items-center gap-8">
                        <div className={`w-16 h-16 rounded-3xl flex items-center justify-center text-2xl font-black shadow-xl transition-transform group-hover:rotate-6 ${
                          percent >= 40 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
                        }`}>
                          {percent}%
                        </div>
                        <div>
                          <p className="text-2xl font-black text-white tracking-tighter uppercase leading-tight">{test?.title || 'System Archive'}</p>
                          <div className="flex items-center gap-3 mt-2">
                             <span className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                                <Calendar size={12} className="text-indigo-500" /> {safeFormat(r.date, 'dd MMM yyyy')}
                             </span>
                             <span className="text-slate-800 font-bold">/</span>
                             <span className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                                <FileText size={12} className="text-purple-500" /> {r.score}/{r.totalQuestions} Units
                             </span>
                          </div>
                        </div>
                     </div>
                     ...
                     <div className="hidden md:block">
                        <div className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border ${
                           percent >= 40 ? 'bg-emerald-500/5 text-emerald-500 border-emerald-500/10' : 'bg-rose-500/5 text-rose-500 border-rose-500/10'
                        }`}>
                          {percent >= 40 ? 'Competency Achieved' : 'Review Suggested'}
                        </div>
                     </div>
                  </div>
                );
              ...
              {studentResults.length === 0 && (
                <div className="py-40 text-center">
                  <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-8 text-slate-800">
                     <Trophy size={40} />
                  </div>
                  <p className="text-slate-600 font-black uppercase tracking-widest text-xs">No assessment results synchronized yet</p>
                </div>
              )}
            </div>
          </motion.div>
